namespace wil034 {
  function largestProduct(array: number[], n = 1) {
    let maxProduct = -Infinity;
    for (let i = 0; i <= array.length - n; i++) {
      const currProduct = array.slice(i, i + n).reduce((p, c) => p * c);
      if (maxProduct < currProduct) maxProduct = currProduct;
    }
